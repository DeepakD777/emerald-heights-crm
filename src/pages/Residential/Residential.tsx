...
type Status = "all" | "available" | "hold" | "booked" | "sold";

type ResidentialFacing = "North" | "South" | "East" | "West";

type FacingFilter = "all" | ResidentialFacing;

type KitchenFilter = "all" | "back-side";
...
const normalizeStatus = (status: PropertyStatus) => {
...
const getUnitNumericValue = (property: Property) => {
...
const getResidentialFacing = (property: Property): ResidentialFacing | "" => {
  ...
  if (property.block === "A Block") {
    return isEven ? "North" : "South";
  }
  ...
  if (property.block === "B Block") {
    return isEven ? "East" : "West";
  }
  ...
  if (property.block === "C1 Tower") {
    return isEven ? "West" : "East";
  }
...
const getResidentialKitchen = (property: Property) => {
  const value = getUnitNumericValue(property);
  ...
  if (property.block === "A Block" && [1, 6, 10].includes(suffix)) {
...
function FlatCard({ property, onClick, className = "", style }: FlatCardProps) {
...
function Residential() {
  ...
  const [selectedKitchen, setSelectedKitchen] = useState<KitchenFilter>("all");
  ...
  const selectedBPhase = selectedSection === "B" ? "Phase 1" : "Phase 2";
  ...
  const loadProperties = async (showLoading = false) => {
    try {
      ...
      if (showLoading) {
        ...
        console.error("Residential auto refresh failed:", err);
      }
    } finally {
      if (showLoading) {
        setLoading(false);
  ...
  useAutoRefresh(() => loadProperties(false), 5000);
  ...
  const blockCounts = useMemo(() => {
    return {
      "A Block": properties.filter((property) => property.block === "A Block")
        .length,

      "B Block": properties.filter((property) => property.block === "B Block")
        .length,

      "C1 Tower": properties.filter((property) => property.block === "C1 Tower")
        .length,
    };
  }, [properties]);
  ...
  const bCount = properties.filter(
    (property) => property.block === "B Block" && property.phase === "Phase 1",
  ).length;

  const b1Count = properties.filter(
    (property) => property.block === "B Block" && property.phase === "Phase 2",
  ).length;
  ...
  const sectionProperties = useMemo(() => {
    return properties
      .filter((property) => {
        ...
        if (property.block !== selectedBlock) {
          return false;
        }

        if (selectedBlock === "B Block") {
      ...
      .sort(naturalSort);
  }, [properties, selectedBlock, selectedBPhase]);
  ...
  const currentFlats = useMemo(() => {
    return sectionProperties
      .filter((property) => Number(property.floor) === selectedFloor)
      .sort(naturalSort);
  }, [sectionProperties, selectedFloor]);
  ...
  const filteredFlats = useMemo(() => {
    ...
    return currentFlats.filter((property) => {
      const status = normalizeStatus(property.status);
      ...
      const matchesFacing =
        selectedFacing === "all" ||
        getResidentialFacing(property) === selectedFacing;

      const matchesKitchen =
        selectedKitchen === "all" ||
        getResidentialKitchen(property) === "Back Side Kitchen";

      return matchesSearch && matchesStatus && matchesFacing && matchesKitchen;
    });
  }, [currentFlats, search, selectedStatus, selectedFacing, selectedKitchen]);

  // ==================================================
  // Statistics
  // ==================================================

  const totalFlats = currentFlats.length;

  const availableFlats = currentFlats.filter(
    (property) => property.status === "AVAILABLE",
  ).length;

  const bookedFlats = currentFlats.filter(
    (property) => property.status === "BOOKED",
  ).length;

  const holdFlats = currentFlats.filter(
    (property) => property.status === "HOLD",
  ).length;

  const soldFlats = currentFlats.filter(
  ...
  const currentTowerName =
    selectedBlock === "A Block"
      ? "Amogh"
      : selectedBlock === "C1 Tower"
        ? "Ishan"
        : selectedSection === "B1"
          ? "B1 Tower"
          : "Ekash";
  ...
  const aBlockFlatMap = useMemo(() => {
    const map = new Map<number, Property>();

    filteredFlats.forEach((property) => {
      const value = getUnitNumericValue(property);
  ...
  const handleBlockChange = (block: ResidentialBlock) => {
  ...
  const handleSectionChange = (section: ResidentialSection) => {
  ...
  const mapPropertyToFlat = (property: Property) => {
    return {
      id: property.id,
      ...
      tower: property.tower || currentTowerName,

      block: property.block ?? "",

      phase: property.phase ?? "",

      facing: getResidentialFacing(property),

      kitchen: getResidentialKitchen(property),
  ...
  const openFlat = (property: Property) => {
    setSelectedFlat(mapPropertyToFlat(property));
  ...
  const handleSaveFlat = async (updatedFlat: any) => {
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to update flat";
  ...
  const handleBooking = async (bookingData: any) => {
    ...
    try {
      await createBooking({
        ...
        propertyId: selectedFlat.propertyId ?? selectedFlat.id,
  ...
  const renderAFlat = (suffix: number) => {
  ...
  const renderC1Flat = (suffix: number) => {
    const property = filteredFlats.find((item) => {
      const value = getUnitNumericValue(item);

      return value !== null && value % 100 === suffix;
    });
  ...
  if (loading) {
    return (
      <div className="rounded-2xl bg-white p-8 shadow">
        <p className="text-gray-600">Loading residential inventory...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-w-0 space-y-4 sm:space-y-5 lg:space-y-6">
      ...
      <div className="min-w-0 rounded-2xl bg-white p-4 shadow sm:p-5 lg:p-6">
        <h2 className="mb-3 text-base font-bold text-gray-800 sm:mb-4 sm:text-lg">
          Select Block / Tower
        </h2>
        ...
        <div className="grid grid-cols-1 gap-2 sm:flex sm:flex-wrap sm:gap-3">
          {RESIDENTIAL_BLOCKS.map((item) => (
            <button
              key={item.value}
              type="button"
              onClick={() => handleBlockChange(item.value)}
              className={`w-full
sm:w-auto
                                        rounded-xl
                                        border
...
sm:py-3
      ...
      {selectedBlock === "B Block" && (
        <div className="min-w-0 rounded-2xl bg-white p-4 shadow sm:p-5 lg:p-6">
          <div className="mb-4">
            <h2 className="text-lg font-bold text-gray-800">Select Section</h2>
...
sm:py-3
                  ...
                  : `
                                ...
                                `}
            >
              B<span className="ml-2 text-xs opacity-80">{bCount} Flats</span>
            </button>

            <button
              type="button"
              onClick={() => handleSectionChange("B1")}
              className={`
                                       w-full
    sm:w-auto
    rounded-xl
    ...
    transition-all
            ...
            >
              B1
              <span className="ml-2 text-xs opacity-80">{b1Count} Flats</span>
            </button>
          </div>
        </div>
      )}
      ...
      <div className="min-w-0 rounded-2xl bg-white p-4 shadow sm:p-5 lg:p-6">
        <div className="mb-5 flex flex-col gap-1 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Tower: {currentTowerName}</p>
          </div>
...
sm:w-auto
      ...
      <div className="grid min-w-0 grid-cols-2 gap-3 sm:gap-4 md:grid-cols-5">
        ...
        <div className="rounded-2xl bg-gray-100 p-4 sm:p-5">
          ...
          <p className="mt-1 text-2xl font-bold text-gray-700 sm:text-3xl">{soldFlats}</p>
        </div>
      </div>
...
================================================== */}
      ...
      <div className="min-w-0 rounded-2xl border border-gray-100 bg-white p-4 shadow sm:p-5">
        <div
          className="
            grid
    min-w-0
        ...
        >
          ...
          <div className="w-full">
            ...
            <select
              ...
              onChange={(event) =>
                setSelectedFacing(event.target.value as FacingFilter)
              }
              className="
                    h-[46px]
                    w-full
          ...
          <div className="w-full">
            ...
            <select
              ...
              onChange={(event) =>
                setSelectedKitchen(event.target.value as KitchenFilter)
              }
              className="
                    h-[46px]
                    w-full
                ...
                "
            >
              <option value="all">All Kitchen</option>

              <option value="back-side">Back Side Kitchen</option>
            </select>
          </div>
